/*
 * xtrica-progress v1.0.1512929297 (https://xtrica.com)
...
if(vm.element){vm.progress=((parseInt(progress)>99)?100:((parseInt(progress)<0)?-1:parseInt(progress)))
if(vm.progress===-1){vm.element.style.opacity=0
setTimeout(()=>{vm.element.style.display='none'
vm.element.style.width='0'
setTimeout(()=>{vm.element.style.display='block'
vm.setProgressActive=!1
resolve()},vm.speed)},vm.speed)}else{vm.element.style.opacity=1
vm.element.style.display='block'
vm.element.style.width=vm.progress+'%'
setTimeout(()=>{vm.setProgressActive=!1
